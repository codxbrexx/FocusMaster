...
test.describe('Registration Page', () => {
  ...
  test('should show step 1 fields by default (name + email)', async ({ page }) => {
  ...
  test('should reject empty step 1 submission', async ({ page }) => {
    await page.getByRole('button', { name: /next/i }).click();
    await expect(page.locator('[class*="bg-red"]').first()).toBeVisible({ timeout: 5000 });
  });

  test('should advance to step 2 with valid name + email', async ({ page }) => {
  ...
  test('should show password strength indicator', async ({ page }) => {
    await page.locator('#name').fill('Test User');
    await page.locator('#email').fill('pw@test.com');
    await page.getByRole('button', { name: /next/i }).click();
  ...
  test('should show passwords match indicator', async ({ page }) => {
    await page.locator('#name').fill('Test User');
    await page.locator('#email').fill('match@test.com');
    await page.getByRole('button', { name: /next/i }).click();
    await page.locator('#password').fill('TestPass123');
  ...
  test('should toggle password visibility', async ({ page }) => {
    await page.locator('#name').fill('Test User');
    await page.locator('#email').fill('toggle@test.com');
    await page.getByRole('button', { name: /next/i }).click();
...
test.describe('Full Registration + Login Flow', () => {
  ...
  test('should register → land on dashboard → logout → login again', async ({ page }) => {
    ...
    // 4. Logout — clear auth and navigate to login
    await page.evaluate(() => {
