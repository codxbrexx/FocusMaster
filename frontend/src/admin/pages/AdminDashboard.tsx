          ...
          >
            <div className="flex justify-between items-start">
              <div>
                ...
                <h3 className="text-2xl font-bold text-foreground mt-1 group-hover:scale-105 transition-transform origin-left">
                  {stat.value}
                </h3>
              </div>
            ...
            <div className="mt-4 flex items-center gap-2 text-xs">
              <span
                className={stat.change.startsWith('+') ? 'text-emerald-500' : 'text-destructive'}
              >
                {stat.change}
              </span>
            ...
            {/* Glow Effect */}
            <div
              className={`absolute -right-6 -bottom-6 w-24 h-24 rounded-full ${stat.bg} blur-2xl opacity-0 group-hover:opacity-40 transition-opacity duration-500 pointer-events-none`}
            />
          </motion.div>
        ))}
      </div>
...
const RecentActivityList = () => {
  ...
  if (error)
    return <div className="text-sm text-destructive text-center py-4">Failed to load activity</div>;
    ...
    <div className="flex flex-col gap-3">
      {activities.map((log, idx) => {
        ...
        return (
          <div
            key={log._id || idx}
            className="flex items-center gap-3 p-3 rounded-xl bg-secondary/20 hover:bg-secondary/40 transition-colors border border-border/30 group"
          >
