...
export const DeviceProvider: React.FC<{ children: React.ReactNode; initialDevice?: DeviceType }> = ({ children, initialDevice }) => {
    ...
    const initialCapabilities = getCapabilities(serverDevice ?? undefined, typeof navigator !== 'undefined' ? navigator : undefined);

    const [state] = useState<DeviceState>(() => ({
        deviceType: serverDevice ?? (typeof navigator !== 'undefined' ? getDeviceType(navigator.userAgent) : 'desktop'),
